...
# Subset the data
subset = AmesHousing[,c(20, 38, 39, 41, 47, 72, 80)]
# Clean subset
...
Fit the model
```{r}
fit = lm(Sale_Price ~ Year_Built + Bsmt_Unf_SF + Total_Bsmt_SF + Gr_Liv_Area
         + Pool_QC + Heating_QC, 
         data = Housing_data)
summary(fit)
```
...
mod3 <- lm(Sale_Price ~ Gr_Liv_Area, data = Housing_data)
plot(residuals(mod3) ~ Housing_data$Gr_Liv_Area, 
     main="Residuals versus Above Grade (Ground) Living Area",
     xlab="Above Grade (Ground) Living Area (Square Feet)", ylab="Residuals")
abline(h=0, col="red", lwd=3, lty=3)
